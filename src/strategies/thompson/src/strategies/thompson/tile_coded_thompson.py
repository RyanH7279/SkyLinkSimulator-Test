...
class TileCodedThompson(Strategy):

    def __init__(self, contexts, distance_precision, no_of_grids):
        self.strategy_name = "tile_coded_thompson" + f"_{int(distance_precision):07}_" + str(no_of_grids)

        # Thompson sampling variables
        self.tiles = {}
        self.posterior_scale = 1

        # context parameters
        self.contexts = contexts
        self.distance_precision = distance_precision * no_of_grids
        self.data_precision_base = 30
        self.time_precision = 3600
        self.total_distance_precision = 1_000_000
        self.elev_precision = 10_000
    ...
    def _calculate_dijkstra_artifacts(self, satellites, groundstations):
        ...
        return distances, dijkstra_targets

    def _build_context(self, sat, target, target_id, current_time, grid_no, distances, dijkstra_targets,
    ...
    def set_targets(self, satellites, groundstations, current_time):
        ...
        distances, dijkstra_targets = self._calculate_dijkstra_artifacts(satellites, groundstations)
        ...
        for sat in satellites:
            ...
            for target_id in np.concatenate((sat.ISL_connections, sat.GSL_connections)):
                ...
                for grid_no in range(self.no_of_grids):
                    ...
                    # smaller sample is better because we minimize routing cost
                    sampled_cost += np.random.normal(
                        avg_cost,
                        self.posterior_scale / math.sqrt(selection_count + 1),
                    )

                targets += [[target_id, sampled_cost / self.no_of_grids]]
